...
#include <string.h>
{
    ...
    if (1)
    {
        i1 = 0;
        i2 = 0;
        int bins_sum_noise = int(0.0005 / sampling_time_f);
        do
        {
        ...
        gr_TRD_noise->Draw();
        TF1 *sin_func = new TF1("sin_func", "[0]*sin(x*TMath::Pi()*100+[1])", time_noise_calc_bin1_f[1], time_noise_calc_bin1_f[int(i2 / bins_sum_noise) - 2]);
        sin_func->SetParLimits(0, 0.015, 0.025);
        // sin_func->SetParLimits(2, 80, 120);
        sin_func->SetParLimits(1, 0, TMath::Pi());
        gr_TRD_noise->Fit(sin_func, "R");
        double noise_par0 = sin_func->GetParameter(0), noise_par1 = sin_func->GetParameter(1);
        // cout << noise_par0 << " " << noise_par1 << endl;
    }
