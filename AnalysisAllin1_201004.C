...
#include <string.h>
{
    ...
    string temp;
    bool flag_display_times_shift, flag_display_raw_data, flag_display_spot_dose, flag_display_cumulated_dose, flag_display_simulated_TRD,
        flag_noise_cancel;
    ...
    if (1)
    {
        i1 = 0;
        i2 = 0;
        int bins_sum_noise = 1; //int(0.0005 / sampling_time_f);
        do
        {
        ...
        gr_TRD_noise->Draw();
        TF1 *noise_func = new TF1("noise_func", "[0]*sin(TMath::Pi()*100*(x+[1]))", time_noise_calc_bin1_f[1], time_noise_calc_bin1_f[int(i2 / bins_sum_noise) - 2]);
        noise_func->SetParLimits(0, 0.015, 0.025);
        noise_func->SetParLimits(1, 0, TMath::Pi());
        gr_TRD_noise->Fit(noise_func, "R");
        double noise_par0 = noise_func->GetParameter(0), noise_par1 = noise_func->GetParameter(1);
        TF1 *noise_cancel_func = new TF1("noise_cancel_func", "[0]*cos(TMath::Pi()*100*(x+[1]))");
    ...
    double noise_integ_max, noise_integ_min;
    double noise_cancel_bin1_d[1000];
    for (int i = 1; i < no_spot; i++)
    {
        noise_cancel_bin1_d[i - 1] = +1 / (100 * twopi * 1.1) * (-noise_cancel_func->Eval(spot_data_bin2_d[0][i]) + noise_cancel_func->Eval(spot_data_bin2_d[0][i - 1])) / sampling_time_f;
